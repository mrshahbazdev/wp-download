...
class WP_Downloader_Admin {
	...
	public static function add_menu() {
		add_management_page(
			__( 'MrShahbazDev Theme & Extension Downloader', 'mrshahbazdev-theme-extension-downloader' ),
			__( 'MrShahbazDev Theme & Extension Downloader', 'mrshahbazdev-theme-extension-downloader' ),
			'manage_options',
			'mrshahbazdev-theme-extension-downloader',
			array( __CLASS__, 'render_page' )
		);
	}

	public static function add_assets( $hook ) {
		if ( 'tools_page_mrshahbazdev-theme-extension-downloader' !== $hook ) {
			return;
		}

		wp_enqueue_style(
			'mrshahbazdev-theme-extension-downloader-admin',
	...
	public static function handle_download() {
		if ( ! isset( $_REQUEST['page'], $_REQUEST['wpd_action'] ) || 'mrshahbazdev-theme-extension-downloader' !== $_REQUEST['page'] ) {
			return;
		}

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to download this item.', 'mrshahbazdev-theme-extension-downloader' ) );
		}

		check_admin_referer( 'wpd_download' );

		if ( ! class_exists( 'ZipArchive' ) ) {
			wp_die( esc_html__( 'ZipArchive extension is not available on this server.', 'mrshahbazdev-theme-extension-downloader' ) );
		}
		...
		if ( 'download' === $action ) {
			if ( ! isset( $_REQUEST['wpd_type'], $_REQUEST['wpd_slug'] ) ) {
				wp_die( esc_html__( 'Missing download parameters.', 'mrshahbazdev-theme-extension-downloader' ) );
			}
		...
		if ( 'bulk_download' === $action ) {
			if ( ! isset( $_REQUEST['wpd_items'] ) || '' === $_REQUEST['wpd_items'] ) {
				wp_die( esc_html__( 'No items selected.', 'mrshahbazdev-theme-extension-downloader' ) );
			}
			...
			if ( empty( $entries ) ) {
				wp_die( esc_html__( 'No valid items were found to download.', 'mrshahbazdev-theme-extension-downloader' ) );
			}

			$zip_name = 'mrshahbazdev-theme-extension-downloader-bulk-' . current_time( 'Ymd-His' );
	...
	private static function resolve_source( $type, $slug ) {
		if ( 'plugin' === $type ) {
			...
			if ( false === $path || false === $base_dir ) {
				return new WP_Error( 'invalid_path', __( 'Invalid plugin path.', 'mrshahbazdev-theme-extension-downloader' ) );
			}
		...
		if ( 'theme' === $type ) {
			...
			if ( ! $theme->exists() ) {
				return new WP_Error( 'not_found', __( 'Theme not found.', 'mrshahbazdev-theme-extension-downloader' ) );
			}
			...
			if ( false === $path || false === $base_dir ) {
				return new WP_Error( 'invalid_path', __( 'Invalid theme path.', 'mrshahbazdev-theme-extension-downloader' ) );
			}
		...
		return new WP_Error( 'invalid_type', __( 'Invalid item type.', 'mrshahbazdev-theme-extension-downloader' ) );
	}
	...
	private static function serve_zip( $entries, $zip_base_name ) {
		...
		if ( true !== $zip->open( $tmp, ZipArchive::CREATE | ZipArchive::OVERWRITE ) ) {
			wp_die( esc_html__( 'Unable to create ZIP file.', 'mrshahbazdev-theme-extension-downloader' ) );
		}
	...
	public static function render_page() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to access this page.', 'mrshahbazdev-theme-extension-downloader' ) );
		}
		...
		$base_url = admin_url( 'tools.php?page=mrshahbazdev-theme-extension-downloader' );
		?>
		<div class="wrap wpd-wrap">
			<div class="wpd-header">
				<h1><?php echo esc_html( get_admin_page_title() ); ?></h1>
				<div class="wpd-toolbar">
					<input type="search" id="wpd-search" placeholder="<?php esc_attr_e( 'Search themes & plugins...', 'mrshahbazdev-theme-extension-downloader' ); ?>" />
				</div>
			</div>
			...
			<form id="wpd-bulk-form" method="post" action="<?php echo esc_url( $base_url ); ?>&wpd_action=bulk_download">
				...
				<div class="wpd-bulk-bar">
					<span><?php esc_html_e( 'Selected:', 'mrshahbazdev-theme-extension-downloader' ); ?> <strong id="wpd-checked-count">0</strong></span>
					<?php submit_button( __( 'Download Selected as ZIP', 'mrshahbazdev-theme-extension-downloader' ), 'primary', 'wpd_bulk_submit', false ); ?>
				</div>
			</form>

			<div class="wpd-tabs">
				<button type="button" class="wpd-tab active" data-target="wpd-themes">
					<?php esc_html_e( 'Themes', 'mrshahbazdev-theme-extension-downloader' ); ?> (<?php echo esc_html( number_format_i18n( count( $themes ) ) ); ?>)
				</button>
				<button type="button" class="wpd-tab" data-target="wpd-plugins">
					<?php esc_html_e( 'Plugins', 'mrshahbazdev-theme-extension-downloader' ); ?> (<?php echo esc_html( number_format_i18n( count( $plugins ) ) ); ?>)
				</button>
			</div>
	...
	private static function render_grid( $items, $type, $active_data, $base_url ) {
		...
		<div class="wpd-toolbar" style="margin-bottom: 16px;">
			<label>
				...
				<?php esc_html_e( 'Select all visible', 'mrshahbazdev-theme-extension-downloader' ); ?>
			</label>
		</div>
		<div class="wpd-grid">
			<?php if ( empty( $items ) ) : ?>
				<div class="wpd-empty"><?php esc_html_e( 'No items found.', 'mrshahbazdev-theme-extension-downloader' ); ?></div>
			<?php else : ?>
				<?php foreach ( $items as $key => $item ) : ?>
					...
					<div class="wpd-card" data-search="<?php echo esc_attr( $search_text ); ?>">
						...
						<div class="wpd-card__meta">
							<?php echo esc_html( $slug ); ?> &bull; <?php esc_html_e( 'Version', 'mrshahbazdev-theme-extension-downloader' ); ?> <?php echo esc_html( $version ); ?>
						</div>
						...
						<div class="wpd-card__footer">
							<span class="wpd-badge <?php echo $active ? 'wpd-badge--active' : 'wpd-badge--inactive'; ?>">
								<?php echo $active ? esc_html__( 'Active', 'mrshahbazdev-theme-extension-downloader' ) : esc_html__( 'Inactive', 'mrshahbazdev-theme-extension-downloader' ); ?>
							</span>
							<a class="button" href="<?php echo esc_url( $download_url ); ?>">
								<?php esc_html_e( 'Download ZIP', 'mrshahbazdev-theme-extension-downloader' ); ?>
							</a>
						</div>
					</div>
				<?php endforeach; ?>
		...
		<div class="wpd-empty" style="display: none;">
			<?php esc_html_e( 'No items match your search.', 'mrshahbazdev-theme-extension-downloader' ); ?>
		</div>
		<?php
	}
}
